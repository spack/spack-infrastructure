...
NEW_SECRET_TEMPLATE = """
# Please modify the secret to your liking here. The data field will be automatically encoded to
...
def handle_create_new_secret():
    value = read_user_input(starting_value=NEW_SECRET_TEMPLATE.lstrip().encode()).strip()
    value = "\n".join([line for line in value.splitlines() if not line.startswith("#")])
    return get_yaml_reader().load(value)
...
def handle_file_update(secrets_file: str, value: str):
    ...
    # If new_secret is True, sealed_secret and secret_index are None
    ...
    if new_secret:
        ...
        sealed_secret = typing.cast("dict", sealed_secret)
        secret_index = typing.cast("int", secret_index)
