...
def create_empty_dimension_rows(apps, schema_editor):
    ...
    GitlabSectionTimerDimension = apps.get_model("core", "GitlabSectionTimerDimension")
    ...
    GitlabSectionTimerDimension.objects.create(
        after_script=0,
...
def reassign_job_fact_dimensions(apps, schema_editor):
    ...
    for job_fact in tqdm(job_facts, total=total):
        ...
        gitlab_section_timers, _ = GitlabSectionTimerDimension.objects.get_or_create(
            after_script=job.gitlab_section_timers.get("after_script", 0),
            cleanup_file_variables=job.gitlab_section_timers.get(
                "cleanup_file_variables", 0
            ),
            download_artifacts=job.gitlab_section_timers.get("download_artifacts", 0),
            get_sources=job.gitlab_section_timers.get("get_sources", 0),
            prepare_executor=job.gitlab_section_timers.get("prepare_executor", 0),
            prepare_script=job.gitlab_section_timers.get("prepare_script", 0),
            resolve_secrets=job.gitlab_section_timers.get("resolve_secrets", 0),
            step_script=job.gitlab_section_timers.get("step_script", 0),
            upload_artifacts_on_failure=job.gitlab_section_timers.get(
                "upload_artifacts_on_failure", 0
            ),
            upload_artifacts_on_success=job.gitlab_section_timers.get(
                "upload_artifacts_on_success", 0
            ),
        )
        ...
        job_fact.gitlab_section_timers = gitlab_section_timers
...
class Migration(migrations.Migration):
    dependencies = [
        ("core", "0003_new_dimensional_models"),
    ]
